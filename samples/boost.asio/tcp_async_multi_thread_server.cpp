/**
 * @file tcp_async_multi_thread_server.cpp
...
class session : public std::enable_shared_from_this<session> {
  ...
  private:
    void do_read_header() {
        ...
        auto read_hander = [this, self](boost::system::error_code ec, std::size_t length){
				if (ec){
		...
		async_read(socket_,boost::asio::buffer(recv_buff_, sizeof(xxx_protocol)),boost::asio::transfer_all(),boost::asio::bind_executor(strand_, read_hander));
    }
	...
	void do_read_payload(uint32_t bytes_to_read){
		...
		async_read(socket_,boost::asio::buffer(recv_buff_ + sizeof(struct xxx_protocol), bytes_to_read),boost::asio::transfer_all(),
            boost::asio::bind_executor(strand_, read_hander));
	}
    ...
    void do_write(const char *buff, std::size_t length) {
        ...
        auto write_handler = [this, self, send_buff](boost::system::error_code ec, std::size_t length) {
                if (ec) {
        ...
        async_write(socket_, boost::asio::buffer(buff, length), boost::asio::bind_executor(strand_,write_handler));
    }
...
class server {
  public:
    server(boost::asio::io_context &io_context, short port) :
        acceptor_(io_context, tcp::endpoint(tcp::v4(), port)),io(io_context) {
  ...
  private:
    void do_accept() {
        ...
        acceptor_.async_accept([this](boost::system::error_code ec, tcp::socket socket) {
            if (!ec) {
                std::make_shared<session>(std::move(socket),io)->start();
				do_accept();
            }else{
